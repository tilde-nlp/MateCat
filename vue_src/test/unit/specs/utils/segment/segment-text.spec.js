...
describe('SegmentText.htmlToXliff', () => {
  it('throws exception on missing value', () => {
    expect(htmlToXliff).toThrow(ValueMissing)
  })
  it('returns same text if no tags inside', () => {
    expect(htmlToXliff('', selfClosingTag.segmentId)).toBe('')
  })
  it('converts one self closed tag', () => {
    const original = selfClosingTag.toHtml()
    expect(htmlToXliff(original, selfClosingTag.segmentId)).toBe(selfClosingTag.toXliff())
  })
  it('converts one dual tag', () => {
    const original = dualOpenTag.toHtml() + dualCloseTag.toHtml()
    const expected = dualOpenTag.toXliff() + dualCloseTag.toXliff()
    expect(htmlToXliff(original, dualOpenTag.segmentId)).toBe(expected)
  })
  it('converts two nested dual tags', () => {
    const secondDualOpenTag = new DualOpenTag()
    secondDualOpenTag.id = 2
    ...
    secondDualCloseTag.name = dualOpenTag.name
    secondDualCloseTag.segmentId = dualOpenTag.segmentId
    const original = dualOpenTag.toHtml() +
      secondDualOpenTag.toHtml() +
      secondDualCloseTag.toHtml() +
      dualCloseTag.toHtml()
    const expected = dualOpenTag.toXliff() +
      secondDualOpenTag.toXliff() +
      secondDualCloseTag.toXliff() +
      dualCloseTag.toXliff()
    expect(htmlToXliff(original, dualOpenTag.segmentId)).toBe(expected)
  ...
  it('converts nested mixed tags', () => {
    const original = dualOpenTag.toHtml() + selfClosingTag.toHtml() + dualCloseTag.toHtml()
    const expected = dualOpenTag.toXliff() + selfClosingTag.toXliff() + dualCloseTag.toXliff()
    expect(htmlToXliff(original, dualOpenTag.segmentId)).toBe(expected)
  })
})
